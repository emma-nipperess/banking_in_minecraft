...
public class WithdrawCommand {
    ...
    private static final String API_URL = "http://10.89.247.191:9000/withdraw-money";
    ...
    private static void addItem(ServerPlayer player, ItemStack itemStack, int count) {
        // Create a copy of the item stack to add
        ItemStack stackToAdd = itemStack.copy();
        stackToAdd.setCount(count);

        // Try to add the stack to the player's inventory
        if (!player.getInventory().add(stackToAdd)) {
            // If unable to add, drop the excess items
    ...
    private static void sendChatMessage(String message, ServerPlayer player) {
        RequestBody body = new FormBody.Builder()
                .add("amount", getValueMap().get(message))
                .build();
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    if (responseBody.startsWith("Fail")) {
                        ...
                        addItem(player, new ItemStack(ModItems.MONEY_ITEMS.get(getItemNameMap().get(message)).get()), 1);
                    }
                } else {
                    player.sendSystemMessage(Component.literal("Server's down please try again."));
                }
